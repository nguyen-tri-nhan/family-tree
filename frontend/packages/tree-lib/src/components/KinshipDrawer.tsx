  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ fontWeight: 800, fontSize: 15, color: 'var(--t-text)', marginBottom: 16, textAlign: 'center' }}>
          Quan hệ họ hàng
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <span style={{ color: 'var(--t-text-4)', fontSize: 18, fontWeight: 300 }}>↔</span>
        ...
        {/* Path */}
        {ab && ab.path.length > 1 && (
          <div style={{ marginTop: 16, paddingTop: 12, borderTop: '1px solid var(--t-border)' }}>
            <div style={sectionLabel}>Đường nối ({ab.path.length - 1} bậc)</div>
            <div style={{ fontSize: 12, color: 'var(--t-text-2)', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 2 }}>
              {pathNames.map((name, i) => (
                <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  <span style={{ fontWeight: i === 0 || i === pathNames.length - 1 ? 700 : 500 }}>{name}</span>
                  {i < pathNames.length - 1 && <span style={{ color: 'var(--t-border-2)', margin: '0 2px' }}>›</span>}
                </span>
              ))}
            </div>
          </div>
...
function PersonChip({ person }: { person: Person }) {
  ...
  return (
    <div style={{ textAlign: 'center', minWidth: 90, maxWidth: 120 }}>
      ...
      <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--t-text)', lineHeight: 1.2 }}>
        {person.displayName}
      </div>
    </div>
  )
...
function RelRow({
  ...
  return (
    <div style={{
      background: 'var(--t-surface)', borderRadius: 8, padding: '10px 12px',
      ...
      <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--t-text)', marginBottom: 3 }}>
        "{label}"
      </div>
      <div style={{ fontSize: 11, color: 'var(--t-text-4)' }}>
        {fromName} tự xưng:{' '}
        <span style={{ fontWeight: 600, color: 'var(--t-text-3)' }}>"{selfLabel}"</span>
      </div>
    </div>
  )
}
...
const modal: React.CSSProperties = {
  background: 'var(--t-card)', borderRadius: 14,
...
const closeBtn: React.CSSProperties = {
  ...
  fontSize: 14, color: 'var(--t-text-4)', padding: 4,
}
